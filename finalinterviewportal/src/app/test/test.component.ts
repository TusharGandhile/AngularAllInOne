...
//('http://interviewapi.ngminds.com/getQuizData')
   ...
   if(localStorage.getItem("newarr")){
    ...
    //  console.log(this.check.questions[i]._id)
            ...
            for (let i = 0; i < this.check.questions.length; i++)(this.check.questions[i]._id==this.nid.qid)?this.currentQuestion=i:""
  ...
  nextQuestion(){
    console.log(this.check.questions[this.currentQuestion]._id)
    this.currentQuestion++;
    // for(let i=0;i< this.check.questions.length;i++){
    //   console.log(this.check.questions[i]._id)}
    //localStorage.setItem('currentQuestion',JSON.stringify(this.currentQuestion))
    this.router.navigate(['/test',this.check._id,this.check.questions[this.currentQuestion]._id])
    this.answer()
